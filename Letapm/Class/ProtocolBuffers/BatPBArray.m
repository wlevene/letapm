...
{
	BatPBArray *copy = [[[self class] allocWithZone:zone] initWithCount:_count valueType:_valueType];
	if (copy)
	{
...
- (NSString *)description
{
	return [NSString stringWithFormat:@"<%@ %p>{valueType = %d, count = %d, capacity = %d, data = %p}",
			[self class], self, _valueType, (int)_count, (int)_capacity, _data];
}
...
{
//	PBArrayValueRangeAssert((long)index);
//	PBArrayValueTypeAssert(PBArrayValueTypeObject);
...
{
//	PBArrayValueRangeAssert((int)index);
...
- (BOOL)isEqualToArray:(BatPBArray *)array
{
	if (self == array)
	{
		return YES;
...
{
	BOOL equal = NO;
	if ([object isKindOfClass:[BatPBArray class]])
	{
...
@implementation BatPBArray (PBArrayExtended)

- (id)arrayByAppendingArray:(BatPBArray *)array
{
	PBArrayValueTypeAssert(array.valueType);

	BatPBArray *result = [[[self class] alloc] initWithCount:_count + array.count valueType:_valueType];
	if (result)
	{
...
@implementation BatPBArray (PBArrayCreation)
...
@implementation BatPBAppendableArray
...
- (void)appendArray:(BatPBArray *)array
{
	PBArrayValueTypeAssert(array.valueType);
