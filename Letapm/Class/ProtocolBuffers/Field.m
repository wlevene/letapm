...
static BatPBField *sDefaultInstance = nil;

+ (void)initialize {
	if (self == [BatPBField class]) {
		sDefaultInstance = [[BatPBField alloc] init];
	}
}
...
- (int32_t)getSerializedSize:(int32_t)fieldNumber {
	...
	for (BatPBUnknownFieldSet *value in _groupArray) {
...
- (void)writeDescriptionFor:(int32_t) fieldNumber
  ...
  for (BatPBUnknownFieldSet* value in self.groupArray) {
    [output appendFormat:@"%@%d: [\n", indent, fieldNumber];
