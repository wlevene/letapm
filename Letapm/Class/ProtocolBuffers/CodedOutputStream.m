...
#import "BatRingBuffer.h"
#import "Message.h"
#import "Utilities.h"
#import "WireFormat.h"


@implementation BatPBCodedOutputStream
...
- (id)initWithOutputStream:(NSOutputStream*)_output data:(NSMutableData*)data {
	if ( (self = [super init]) ) {
		...
		buffer = [[BatRingBuffer alloc] initWithData:data];
	}
	return self;
}
...
+ (BatPBCodedOutputStream*)streamWithOutputStream:(NSOutputStream*)output bufferSize:(int32_t)bufferSize {
	NSMutableData *data = [NSMutableData dataWithLength:bufferSize];
	return [[[BatPBCodedOutputStream alloc] initWithOutputStream:output data:data] autorelease];
}


+ (BatPBCodedOutputStream*)streamWithOutputStream:(NSOutputStream*)output {
	return [BatPBCodedOutputStream streamWithOutputStream:output bufferSize:DEFAULT_BUFFER_SIZE];
}


+ (BatPBCodedOutputStream*)streamWithData:(NSMutableData*)data {
	return [[[BatPBCodedOutputStream alloc] initWithOutputStream:nil data:data] autorelease];
}
...
- (void)writeGroupNoTag:(int32_t)fieldNumber value:(const id<BatPBMessage>)value {
...
- (void)writeGroup:(int32_t)fieldNumber value:(const id<BatPBMessage>)value {
	[self writeTag:fieldNumber format:PBWireFormatStartGroup];
	[self writeGroupNoTag:fieldNumber value:value];
}


- (void)writeUnknownGroupNoTag:(int32_t)fieldNumber value:(const BatPBUnknownFieldSet*)value {
	[value writeToCodedOutputStream:self];
	[self writeTag:fieldNumber format:PBWireFormatEndGroup];
}
...
- (void)writeUnknownGroup:(int32_t)fieldNumber value:(const BatPBUnknownFieldSet*)value {
	[self writeTag:fieldNumber format:PBWireFormatStartGroup];
	[self writeUnknownGroupNoTag:fieldNumber value:value];
}


- (void)writeMessageNoTag:(const id<BatPBMessage>)value {
...
- (void)writeMessage:(int32_t)fieldNumber value:(const id<BatPBMessage>)value {
	[self writeTag:fieldNumber format:PBWireFormatLengthDelimited];
 ...
 */
- (void)writeMessageSetExtension:(int32_t)fieldNumber value:(const id<BatPBMessage>)value {
