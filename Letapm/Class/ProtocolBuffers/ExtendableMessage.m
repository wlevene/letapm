...
@implementation BatPBExtendableMessage
...
- (BOOL) isInitialized:(id) object {
  ...
  } else if ([object conformsToProtocol:@protocol(BatPBMessage)]) {
...
- (void) writeExtensionsToCodedOutputStream:(BatPBCodedOutputStream*) output
...
- (BOOL) isEqualExtensionsInOther:(BatPBExtendableMessage*)otherMessage
                             from:(int32_t) startInclusive
                               to:(int32_t) endExclusive {
  NSArray* sortedKeys = [extensionMap.allKeys sortedArrayUsingSelector:@selector(compare:)];
  for (NSNumber* number in sortedKeys) {
