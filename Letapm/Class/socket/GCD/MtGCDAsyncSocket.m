...
#ifndef MtGCDAsyncSocketLoggingEnabled
#define MtGCDAsyncSocketLoggingEnabled 0
#endif

#if MtGCDAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext MtGCDAsyncSocketLoggingContext
...
#ifndef MtGCDAsyncSocketLogLevel
#define MtGCDAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = MtGCDAsyncSocketLogLevel;
...
NSString *const MtGCDAsyncSocketException = @"MtGCDAsyncSocketException";
NSString *const MtGCDAsyncSocketErrorDomain = @"MtGCDAsyncSocketErrorDomain";

NSString *const MtGCDAsyncSocketQueueName = @"MtGCDAsyncSocketQueueName";
NSString *const MtGCDAsyncSocketThreadName = @"MtGCDAsyncSocketThreadName-CFStream";

NSString *const MtGCDAsyncSocketManuallyEvaluateTrust = @"MtGCDAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const MtGCDAsyncSocketUseCFStreamForTLS = @"MtGCDAsyncSocketUseCFStreamForTLS";
#endif
NSString *const MtGCDAsyncSocketSSLPeerID = @"MtGCDAsyncSocketSSLPeerID";
NSString *const MtGCDAsyncSocketSSLProtocolVersionMin = @"MtGCDAsyncSocketSSLProtocolVersionMin";
NSString *const MtGCDAsyncSocketSSLProtocolVersionMax = @"MtGCDAsyncSocketSSLProtocolVersionMax";
NSString *const MtGCDAsyncSocketSSLSessionOptionFalseStart = @"MtGCDAsyncSocketSSLSessionOptionFalseStart";
NSString *const MtGCDAsyncSocketSSLSessionOptionSendOneByteRecord = @"MtGCDAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const MtGCDAsyncSocketSSLCipherSuites = @"MtGCDAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const MtGCDAsyncSocketSSLDiffieHellmanParameters = @"MtGCDAsyncSocketSSLDiffieHellmanParameters";
#endif

enum MtGCDAsyncSocketFlags
{
...
@interface MtGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation MtGCDAsyncSocketPreBuffer
...
**/
@interface MtGCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(MtGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation MtGCDAsyncReadPacket
...
**/
@interface MtGCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation MtGCDAsyncWritePacket
...
**/
@interface MtGCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation MtGCDAsyncSpecialPacket
...
@implementation MtGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	...
	MtGCDAsyncReadPacket *currentRead;
	MtGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	MtGCDAsyncSocketPreBuffer *preBuffer;
	...
	MtGCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([MtGCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[MtGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak MtGCDAsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong MtGCDAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			MtGCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
                ...
                error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		#pragma clang diagnostic warning "-Wimplicit-retain-self"
			...
			NSMutableArray *addresses = [MtGCDAsyncSocket lookupHost:hostCpy port:port error:&lookupErr];
			...
			else
			{
				...
				{
					if (!address4 && [MtGCDAsyncSocket isIPv4Address:address])
					{
						address4 = address;
					}
					else if (!address6 && [MtGCDAsyncSocket isIPv6Address:address])
					{
						address6 = address;
					}
				}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MtGCDAsyncSocketErrorDomain code:MtGCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MtGCDAsyncSocketErrorDomain code:MtGCDAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MtGCDAsyncSocketConnectTimeoutError",
	                                                     @"MtGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MtGCDAsyncSocketErrorDomain code:MtGCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MtGCDAsyncSocketReadMaxedOutError",
														 @"MtGCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MtGCDAsyncSocketErrorDomain code:MtGCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MtGCDAsyncSocketReadTimeoutError",
	                                                     @"MtGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MtGCDAsyncSocketErrorDomain code:MtGCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MtGCDAsyncSocketWriteTimeoutError",
	                                                     @"MtGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MtGCDAsyncSocketErrorDomain code:MtGCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MtGCDAsyncSocketClosedError",
	                                                     @"MtGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MtGCDAsyncSocketErrorDomain code:MtGCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MtGCDAsyncSocketErrorDomain code:MtGCDAsyncSocketOtherError userInfo:userInfo];
}
                        ...
                        tag:(long)tag
{
	...
	MtGCDAsyncReadPacket *packet = [[MtGCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[MtGCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[MtGCDAsyncSpecialPacket class]])
			{
...
- (void)completeCurrentRead
{
	...
	{
		MtGCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		MtGCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	MtGCDAsyncWritePacket *packet = [[MtGCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[MtGCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[MtGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued GCDAsyncSpecialPacket");
...
- (void)doWriteTimeout
{
	...
	{
		MtGCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	MtGCDAsyncSpecialPacket *packet = [[MtGCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			MtGCDAsyncSpecialPacket *tlsPacket = (MtGCDAsyncSpecialPacket *)currentRead;
			NSDictionary *tlsSettings = tlsPacket->tlsSettings;
			
			NSNumber *value = [tlsSettings objectForKey:MtGCDAsyncSocketUseCFStreamForTLS];
...
{
	MtGCDAsyncSocket *asyncSocket = (__bridge MtGCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:MtGCDAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  9. MtGCDAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:MtGCDAsyncSocketSSLPeerID];
	...
	value = [tlsSettings objectForKey:MtGCDAsyncSocketSSLProtocolVersionMin];
	...
	value = [tlsSettings objectForKey:MtGCDAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MtGCDAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for MtGCDAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. GCDAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:MtGCDAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MtGCDAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for MtGCDAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. GCDAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:MtGCDAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MtGCDAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for MtGCDAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. GCDAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:MtGCDAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MtGCDAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for MtGCDAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. MtGCDAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:MtGCDAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MtGCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for MtGCDAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use MtGCDAsyncSocketSSLProtocolVersionMin & MtGCDAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[MtGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"MtGCDAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[MtGCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[MtGCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("MtGCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
+ (void)stopCFStreamThreadIfNeeded
{
	...
	#pragma clang diagnostic warning "-Wimplicit-retain-self"
		...
		if (--cfstreamThreadRetainCount == 0)
		{
			...
			// wake up the thread
			[MtGCDAsyncSocket performSelector:@selector(ignore:)
...
{
	[[NSThread currentThread] setName:MtGCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(MtGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(MtGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	MtGCDAsyncSocket *asyncSocket = (__bridge MtGCDAsyncSocket *)pInfo;
